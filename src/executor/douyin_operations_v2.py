...
from core import create_automation
from config.douyin_element_ids import DouyinElementIds, DeviceElementVersion
import time
...
class DouyinOperations:
    ...
    def _get_element_id(self, element_config):
        ...
        Returns:
            str: 该设备应使用的元素ID
        """
        return self.element_version.get_element_id(element_config, self.auto.device)
    ...
    def find_and_click_search_button(self):
        ...
        try:
            ...
            if self.auto.element_exists(resourceId=search_button_id):
                element = self.auto.device(resourceId=search_button_id)
        ...
        for swipe_count in range(1, max_swipes + 1):
            ...
            self.auto.device.swipe(center_x, start_y, center_x, end_y, duration=0.3)
            time.sleep(1)
    ...
    def find_search_input(self):
        ...
        try:
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.SEARCH_INPUT)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.SEARCH_INPUT))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def find_search_confirm_button(self):
        ...
        try:
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.SEARCH_CONFIRM)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.SEARCH_CONFIRM))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def switch_to_user_tab(self):
        ...
        # 使用模板匹配查找用户标签（阈值0.6）
        result = self.auto.find_image(user_tab_icon, method="template", threshold=0.6)
    ...
    def find_first_user_result(self):
        ...
        try:
            ...
            if not self.auto.element_exists(resourceId=element_id):
            ...
            elements = self.auto.device(resourceId=element_id)
    ...
    def find_pinned_video(self):
        ...
        try:
            ...
            if self.auto.element_exists(resourceId=video_element_id):
                elements = self.auto.device(resourceId=video_element_id)
    ...
    def find_comment_button(self):
        ...
        try:
            ...
            if self.auto.element_exists(resourceId=element_id):
                element = self.auto.device(resourceId=element_id)
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def find_comment_input(self):
        ...
        try:
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.COMMENT_INPUT)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.COMMENT_INPUT))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def click_image_comment_button(self):
        ...
        try:
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.IMAGE_COMMENT_ICON)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.IMAGE_COMMENT_ICON))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def select_album_image(self):
        ...
        try:
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.ALBUM_FIRST_IMAGE)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.ALBUM_FIRST_IMAGE))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def click_image_comment_send(self):
        ...
        try:
            ...
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.SEND_IMAGE_COMMENT)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.SEND_IMAGE_COMMENT))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def find_send_button(self):
        ...
        try:
            if self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.SEND_TEXT_COMMENT)):
                element = self.auto.device(resourceId=self._get_element_id(DouyinElementIds.SEND_TEXT_COMMENT))
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def click_user_page_back(self):
        ...
        try:
            if self.auto.element_exists(resourceId="com.ss.android.ugc.aweme:id/jyz"):
                element = self.auto.device(resourceId="com.ss.android.ugc.aweme:id/jyz")
                bounds = element.info['bounds']
                x = (bounds['left'] + bounds['right']) // 2
                y = (bounds['top'] + bounds['bottom']) // 2
    ...
    def check_is_homepage(self):
        ...
        try:
            ...
            if self.auto.element_exists(resourceId=bottom_nav_home_id):
            ...
            try:
                ...
                nav_elements = self.auto.device(resourceId=bottom_nav_common_id)
            ...
            if fragment_container_id and self.auto.element_exists(resourceId=fragment_container_id):
    ...
    def check_is_search_page(self):
        ...
        try:
            # 检测搜索按钮
            has_search_btn = self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.SEARCH_CONFIRM))
            # 检测搜索输入框
            has_search_input = self.auto.element_exists(resourceId=self._get_element_id(DouyinElementIds.SEARCH_INPUT))
    ...
    def check_for_private_account(self):
        ...
        try:
            # 检测私密账户标题元素
            if self.auto.element_exists(resourceId="com.ss.android.ugc.aweme:id/title"):
                element = self.auto.device(resourceId="com.ss.android.ugc.aweme:id/title")
...
def create_douyin_operations(auto=None, device_id=None, template_manager=None, device_model=None):
    ...
    """
    if auto is None:
        auto = create_automation(device_id=device_id, enable_ocr=True)
